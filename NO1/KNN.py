...
trainingDigits =r'D:\project_python\ClassLearning\NO1\digits\trainingDigits'
testDigits = r'D:\project_python\ClassLearning\NO1\digits\testDigits'
                                                            ## ↑数据路径
...
def read_file(doc_name):                                    ## 定义一个把32x32格式转为1行的函数
    ...
    for i in range(32):                                     ## 已知每个文件中有32行32列
        ...
        for j in range(32):                                 ## 取每行中的每一列
            data[0,32*i+j]=int(float(hang[j]))              ## 给data值
